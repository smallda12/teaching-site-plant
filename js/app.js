...
(function () {
  ...
  const 相簿 = (function () {
    ...
    function 開() {
      return new Promise((res) => {
        if (db) return res(db);
        const r = indexedDB.open("植物小百科照片", 1);
  ...
  const 點名 = (function () {
    ...
    async function 匯出() {
      ...
      const 資料 = { 格式: "植物小百科點名名單", 版本: 1, 匯出時間: new Date().toISOString(), 學生: [] };
      for (const s of 名單) {
    ...
    function 畫教學頁() {
      ...
      $("#重點內容").innerHTML = `
        ...
        <div class="按鈕列 置中">
          ...
          <button class="主要鈕" id="鈕下步">${步驟 < 目前重點.教學頁.length - 1 ? "下一頁 →" : (目前重點.短片 ? "看主題短片 🎬" : "開始練習 ✏️")}</button>
        </div>`;
      $("#鈕回重點").onclick = 離開;
